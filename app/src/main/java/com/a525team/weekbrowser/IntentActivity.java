...
public class IntentActivity extends Activity {
	...
	private void initializeLogic() {
		Skin.getInstance().colorToRam(this);
		...
		if (intent2.getData().toString().startsWith("https://skins.weekbrowser.com/bookmarks")) {
			...
			if (intent2.getData().toString().startsWith("https://multilink2.weekbrowser.com/")) {
				...
				RenderUtils.setupDialogDim(IntentActivity.this, dddddd);
				dddddd.show();
				...
				if (intent2.getData().toString().startsWith("https://skins.weekbrowser.com/general")) {
					...
					if (intent2.getData().toString().startsWith("https://extensions.weekbrowser.com")) {
						...
						if (intent2.getData().toString().startsWith("https://colorpicker.weekbrowser.com")) {
							ColorPickerDialog colorPickerDialog = new ColorPickerDialog(IntentActivity.this);
							...
							if (intent2.getData().toString().startsWith("https://texteditor.weekbrowser.com")) {
								thema.setClass(getApplicationContext(), TextActivity.class);
								thema.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
								startActivity(thema);
								finish();
							} else {
								AdBlocker adBlocker = AdBlocker.getInstance(this);
								int adN = 0;
								// String url = filePath;
								try{
									if (adBlocker.blockRedir()) {
										adN = adBlocker.isBlocked(intent2.getData().toString());
										isAd = adN != 0;
									}} catch (Exception e) {
