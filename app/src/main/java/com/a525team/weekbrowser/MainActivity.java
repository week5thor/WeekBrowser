							...
							if (ticks%20 == 0) {
								...
								new Thread(new Runnable() {
									...
									public void run() {
										...
										memoryUsageText = "UsedRAM: " + usedMemoryMB + "MiB";
...
*/
	...
	private void openOrRestoreTab(int position) {
		...
		try {
			...
			// Якщо тег не знайдено або був null, створюємо/відновлюємо вкладку
			if (!tabExists) {
				if (tab.get(position).get("url") != null) {
					...
					// Встановлюємо новий ідентифікатор вкладки
					tab.get(position).put("tab", Tt);
					...
					newWebView.loadUrl(tab.get(position).get("url").toString());
					...
					newWebView.loadUrl(sf.getString("firstpage", ""));
					...
					tab.get(position).put("tab", Tt);
					tab.get(position).put("url", sf.getString("firstpage", ""));
				}
			}
			
			// Оновлюємо час та активну вкладку
			tab.get(position).put("time", time);
			((BaseAdapter) tabscont.getAdapter()).notifyDataSetChanged();
			sf.edit().putString("activetab", String.valueOf(position)).commit();
			sf.edit().putString("tabs", JsonUtils.toJson(tab)).commit();
			...
			.setPrimaryClip(ClipData.newPlainText("clipboard", e.toString()));
		}
		...
		if((int)_a < 0){
			try{
				switch((int)_a){
							...
							while (!stack.isEmpty()) {
								...
								for (int h = 0; h < parent.getChildCount(); h++) {
									...
									if (child instanceof LinearLayout && child.getTag() != null && child.getTag().toString().startsWith("ll")) {
										...
										linearLayout.getLayoutParams().width = ViewGroup.LayoutParams.MATCH_PARENT;
										linearLayout.getLayoutParams().height = ViewGroup.LayoutParams.MATCH_PARENT;
										((LinearLayout.LayoutParams) linearLayout.getLayoutParams()).weight = 1;
										linearLayout.requestLayout();
										linearLayout.setX(0);
										linearLayout.setY(0);
					...
					case -18:
					showBlockAdsDialog(sf, _position, tabUrl);
					break;
			...
			try {
				...
				WebView targetWebView = findWebViewByTag(tabTag);
	...
	private void showBlockAdsDialog(SharedPreferences sf, int position, String url) {
		try {
			AlertDialog.Builder blockads = new AlertDialog.Builder(this);
			blockads.setTitle("?");
			blockads.setMessage(getString(R.string.blocksite1) + "\n" + 
			_extractDomain(url) + getString(R.string.blocksite2) + url);
			
			blockads.setPositiveButton("[    1    ]", (dialog, which) -> {
				String adhosts = sf.getString("adhosts", "") + "\n" + 
				_extractDomain(url).replace(".", "\\.");
				sf.edit().putString("adhosts", adhosts).commit();
				_closetab(position);
			});
			
			blockads.setNegativeButton("[    2    ]", (dialog, which) -> {
				String adhosts = sf.getString("adhosts", "") + "\n%" + 
				url.replace(".", "\\.");
				sf.edit().putString("adhosts", adhosts).commit();
				_closetab(position);
			...
			blockads.setNeutralButton(getString(R.string.cancel), null);
			
			AlertDialog dialog = blockads.create();
			setupDialogDim(dialog);
			dialog.show();
			
		} catch(Exception e) {
			SketchwareUtil.showMessage(getApplicationContext(), "error");
			copyToClipboard(e.toString(), 0);
		}
	}
