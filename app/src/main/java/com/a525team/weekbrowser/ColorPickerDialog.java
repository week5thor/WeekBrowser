...
*/
    ...
    // Старий код не зламається, бо цей метод вже має порожню реалізацію за замовчуванням
	...
	public void show(String colhex) {
		...
		// Додаємо масив з одним елементом, щоб обійти обмеження effectively final всередині лямбд
...
ok.setOnClickListener(view -> {
    isOkClicked[0] = true; // Фіксуємо, що натиснуто саме кнопку "Ок"
...
// Слухач, який спрацює при БУДЬ-ЯКОМУ закритті діалогу
dialog2.setOnDismissListener(dialogInterface -> {
    // Якщо діалог закрився, але кнопку "Ок" не натискали — викликаємо onCancel()
    if (!isOkClicked[0] && listener != null) {
        listener.onCancel();
    }
});
