...
public class ShortcutActivity extends Activity {
	...
	private SkinTextView nam;
	private SkinEditText name;
	private SkinTextView ur;
	private SkinEditText url;
				...
				try{
					...
					if(WTF.SDK_INT() >= Build.VERSION_CODES.O) {
						...
						if (shortcutManager != null) {
							...
							// Отримуємо максимально допустимий розмір для іконки
							Bitmap resizedBmp = resizeBitmap(bmp, 96, 96);
							...
							.setIcon(Icon.createWithBitmap(resizedBmp)) // Використовуємо масштабований бітмап
							.build();
						...
						addIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON, resizedBmp);
	...
	private void initializeLogic() {
		...
		Skin.getInstance().setBG(findViewById(android.R.id.content), 2, false);
...
insmlink.setTextColor((int)Long.parseLong(dataHolder.getData("bt"),16));
	...
	private Bitmap resizeBitmap(Bitmap bitmap, int maxWidth, int maxHeight) {
		...
		if (scale >= 1.0f) return bitmap; // Якщо іконка менша, повертаємо її як є
