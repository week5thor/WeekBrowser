...
public class BarMan {
	...
	private void initActionConfigs() {
		...
		addAction(-9, R.string.deeprefresh, " !");
		addAction(-10, R.string.newtabbtn, "");
		addAction(-11, R.string.background_tab, "");
		
		addAction(-13, R.string.close_other, "[]");
		addAction(-14, R.string.dup_tab, "[]");
		addAction(-15, R.string.unload_tab, "");
		addAction(-16, R.string.unload_other, "[]");
		addAction(-17, R.string.create_shortcut, "");
		addAction(-18, R.string.close_block, "");
		addAction(-19, R.string.copy_name, "N");
		addAction(-20, R.string.copy_url, "U");
		addAction(-21, R.string.copy_nameurl, "NU");
		addAction(-22, R.string.share_name, "N");
		addAction(-23, R.string.share_url, "U");
		addAction(-24, R.string.share_nameurl, "NU");
		addAction(-25, R.string.load_all, "[[]]");
        addAction(-29, R.string.close_all_tabs, "");
	...
	public ArrayList<HashMap<String, Object>> sfToArray(String config) {
		...
		// Спочатку замінимо екрановані символи, щоб вони не сприймалися як роздільники
		// Але ми не можемо просто замінити %25 на %, бо це зруйнує розділення.
		// Натомість використаємо спеціальні тимчасові маркери
		config = config.replace("%25", "\uE001"); // тимчасовий маркер для %
		config = config.replace("%26", "\uE002"); // тимчасовий маркер для &
		
		String[] buttons = config.split("%");
		
		for (String btn : buttons) {
			if (btn.isEmpty()) continue;
			String[] parts = btn.split("&", -1); // Використовуємо -1 для збереження порожніх значень
			HashMap<String, Object> map = new HashMap<>();
			
			// Відновлюємо оригінальні символи з тимчасових маркерів
			if (parts.length > 0) map.put("click", restoreEscaped(parts[0]));
			if (parts.length > 1) map.put("long", restoreEscaped(parts[1]));
			if (parts.length > 2) map.put("icon", restoreEscaped(parts[2]));
			if (parts.length > 3) map.put("clickarg", restoreEscaped(parts[3]));
			if (parts.length > 4) map.put("longarg", restoreEscaped(parts[4]));
	...
	public String arrayToSf(ArrayList<HashMap<String, Object>> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			result.append(escape(getStringValue(btn.get("click")))).append("&")
			.append(escape(getStringValue(btn.get("long")))).append("&")
			.append(escape(getStringValue(btn.get("icon")))).append("&")
			.append(escape(getStringValue(btn.get("clickarg")))).append("&")
			.append(escape(getStringValue(btn.get("longarg"))));
		}
		return result.toString();
	}
	
	// Допоміжні методи для екранування
	private String escape(String value) {
		if (value == null) return "";
		// Екрануємо спочатку %, потім &
		return value.replace("%", "%25").replace("&", "%26");
	}
	
	private String restoreEscaped(String value) {
		if (value == null) return "";
		// Відновлюємо з тимчасових маркерів
	...
	private void addButtonToLayout(HashMap<String, Object> config) {
		...
		try{
			...
			// Додаємо крапку лише якщо є довгий клік і це дозволено налаштуваннями
			Object longAction = config.get("long");
			boolean hasLongAction = longAction != null && !"-990".equals(longAction.toString());
			boolean showDot = sf.getString("toolbardot", "1").equals("1");
			
			String displayText = icon;
			if (hasLongAction && showDot) {
				displayText = icon + "•";
			}
	...
	private void setupButtonAppearance(SkinButton button, boolean invis, int buttonIndex) {
		float baseSizeDp = Float.parseFloat(sf.getString("button", "40"));
		float density = activity.getResources().getDisplayMetrics().density;
...
button.setLayoutParams(lp);*/		
		...
		((BarsActivity) activity)._getBtnNum() == buttonIndex){
			button.setSkinKey("rbg", true);
		}else{
			if (invis) {
				button.setSkinKey("add", true);
			} else {
				button.setSkinKey("bbg", true); // або "bbg" чи "rbg"
			}
		}
	}
	...
	private void updateButtonsAppearance() {
		for (int i = 0; i < layout.getChildCount(); i++) {
			...
			if (child instanceof SkinButton) {
				SkinButton button = (SkinButton) child;
				...
				if (child instanceof Button) {
					...
					if(tag != null && !tag.isEmpty()){
						...
						if (currentText.contains("?tabs?")) {
							...
							// Додаємо крапку лише якщо є довгий клік і це дозволено налаштуваннями
							boolean hasLongAction = longC != null && !"-990".equals(longC);
							boolean showDot = sf.getString("toolbardot", "1").equals("1");
							if (hasLongAction && showDot) {
								newText += "•";
							}
