...
public class TextActivity extends Activity {
		...
		save.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (isFile) {
					...
					try {
						HashMap<String, Object> temp2 = JsonUtils.M(
	...
	private void initializeLogic() {
		...
		for(int i=0;i<300;i++)hhhh.append("\n");
		...
		settings.setTextColor((int) Long.parseLong(dataHolder.getData("bt"), 16));
		skin.setBG(settings, 0, false);
	...
	protected void onPostCreate(Bundle _savedInstanceState) {
		...
		new Thread(() -> {
			...
			if (getIntent().hasExtra("dh")) {
				...
				try{temp2 = JsonUtils.M(dataHolder.getData("info"));
				}catch(Exception e){}
	...
	public void _read() {
		...
		if(text.length() > 2000000){
			isCut = true;
			editText.setText(text.toString().substring(0, 2000000)); 
		}else{
		...
		name.setText((isCut ? " " : "") +
		"[" +
		Formater.kmgt((int) editText.getText().toString().length()) +
		"] " +
