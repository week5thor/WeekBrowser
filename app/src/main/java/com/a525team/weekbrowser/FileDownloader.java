...
public class FileDownloader {
	...
	// ДОДАНО: Змінна для зберігання вибраного шляху
	...
	public FileDownloader(Activity activity, WebView webView) {
		...
		// Встановлюємо шлях за замовчуванням (стандартна папка Downloads)
		this.currentDownloadPath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath();
	...
	private void showDownloadDialog(String url, String userAgent, String contentDisposition, String mimeType, long contentLength, String base64DataForBlob) {
		...
		// ДОДАНО: Блок вибору шляху для збереження
		TextView pathTitle = new TextView(activity);
		pathTitle.setText(activity.getString(R.string.save_path)); // Можете замінити на getString(R.string.save_path)
		pathTitle.setPadding(0, 20, 0, 5);
		layout.addView(pathTitle);
		
		TextView pathButton = new TextView(activity);
		pathButton.setText(currentDownloadPath);
		pathButton.setPadding(30, 30, 30, 30);
		pathButton.setTextSize(14f);
		pathButton.setMaxLines(2);
		pathButton.setEllipsize(android.text.TextUtils.TruncateAt.MIDDLE);
		
		// Стилізуємо під Spinner/EditText за допомогою системного атрибута
		android.util.TypedValue outValue = new android.util.TypedValue();
		activity.getTheme().resolveAttribute(android.R.attr.editTextBackground, outValue, true);
		pathButton.setBackgroundResource(outValue.resourceId);
		layout.addView(pathButton);
		
		// У методі showDownloadDialog:
		
		pathButton.setOnClickListener(v -> {
			// Передаємо назву кнопки "ОК", якщо це вибір папки
			BasedDialog.showFileManager(activity, "Виберіть папку", "", (byte)0x02, file -> {
				if (file != null) {
					// Оновлюємо наш глобальний шлях
					currentDownloadPath = file.getAbsolutePath();
					// Оновлюємо UI кнопки
					pathButton.setText(currentDownloadPath);
					// Примусово оновлюємо вигляд
					pathButton.invalidate(); 
				}
			});
		});
		// КІНЕЦЬ БЛОКУ вибору шляху
		...
		RenderUtils.setupDialogDim(activity, dialog2);
		dialog2.show();
	}
	...
	private void downloadHttpFile() {
		...
		// ДОДАНО: Використовуємо вибраний користувачем шлях замість стандартного
		File destFile = new File(currentDownloadPath, fileName);
	...
	private void saveFile(byte[] data, String fileName, String mimeType) throws IOException {
		// ДОДАНО: Використовуємо вибраний користувачем шлях для ручного збереження (Blob/Base64)
