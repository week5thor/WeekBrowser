...
public class MyWebViewClient extends WebViewClient {
	...
	private void injectHistoryListener(WebView view) {
    String js = "javascript:(function() {"
        + "if (window.__historyListenerAdded) return;"
        + "window.__historyListenerAdded = true;"
        + "window.__lastHref = location.href;"
        + "window.__lastTitle = document.title;"
        + "window.__changeHistory = function() {"
        + "    setTimeout(function() {"
        + "        if (window.__lastHref !== location.href) {"
        + "            window.__lastHref = location.href;"
        + "            window.__lastTitle = document.title;"
        + "            console.log('URL змінився:', location.href);"
        + "        }"
        + "    }, 50);"
        + "};"
        + "history.pushState = (f => function pushState() { f.apply(this, arguments); window.__changeHistory(); })(history.pushState);"
        + "history.replaceState = (f => function replaceState() { f.apply(this, arguments); window.__changeHistory(); })(history.replaceState);"
        + "window.addEventListener('popstate', window.__changeHistory);"
        + "window.addEventListener('hashchange', window.__changeHistory);"
        + "var observer = new MutationObserver(function(mutations) {"
        + "    for (var i = 0; i < mutations.length; i++) {"
        + "        if (mutations[i].addedNodes.length > 0) {"
        + "            console.log('Відбулися зміни на сторінці');"
        + "            if (window.__lastHref !== location.href) {"
        + "                console.log('Справжній перехід: перезавантажуємо сторінку');"
        + "                location.reload();"
        + "            }"
        + "            break;"
        + "        }"
        + "    }"
        + "});"
        + "var targetNode = document.querySelector('#page') || document.querySelector('#content');"
