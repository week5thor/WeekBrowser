...
public class JSutil {
	...
	private static String generatePattern(ExtendedDataHolder dh, String type) {
		...
		if (dh.getData("batsav").equals("1") || WTF.SDK_INT() < 21){
			switch(type){
				case "bg": return "#"+_toHexString(Skin.getInstance().avgbg()).substring(3,9);
				case "button": return "#"+_toHexString(Skin.getInstance().avgb()).substring(3,9);
				case "red": return "#"+_toHexString(Skin.getInstance().avgrb()).substring(3,9);
			}
		}
	...
	private static String _toHexString(int _num){
        String lol;
		if (_num < 16) {
			lol="0".concat(Integer.toHexString(_num));
		} else {
			lol=Integer.toHexString(_num);
		}
		return lol;
	}
