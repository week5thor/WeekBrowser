...
public class SsuperactivityActivity extends Activity {
	...
	private SkinTextView prop;
	private LinearLayout linear3;
	private LinearLayout linear10;
	private WebView webview1;
	private LinearLayout linear8;
	private SkinButton changbg;
	private SkinButton third;
	private SkinButton half;
	private SkinButton twothird;
	private SkinButton full;
	private ScrollView vscroll1;
	private ScrollView vscroll2;
	private LinearLayout linear1;
	private LinearLayout linear5;
	private SkinButton topage;
	private SkinTextView textareaid;
	private SkinEditText txtarea;
	private SkinButton txtready;
	private SkinTextView a1;
	private SkinAutoCompleteTextView img;
	private SkinButton downloadImageButton;
	private LinearLayout linear6;
	private SkinCheckBox incognitosrc;
	private LinearLayout srcbuttons2;
	private SkinButton ad;
	private SkinTextView a2;
	private SkinEditText href;
	private LinearLayout urlbuttons;
	private SkinCheckBox incognitohref;
	private LinearLayout callbuttons;
	private LinearLayout urlbuttons2;
	private SkinTextView a5;
	private SkinEditText eltext;
	private LinearLayout textutils;
	private SkinTextView a3;
	private SkinAutoCompleteTextView elementhtml;
	private SkinButton add;
	private SkinButton temproarystd;
	private SkinButton sametab;
	private SkinButton bgtab;
	private SkinButton copysrc;
	private SkinButton button1;
	private SkinButton hnew;
	private SkinButton hthis;
	private SkinButton hbg;
	private SkinButton call;
	private SkinButton sms;
	private SkinButton copyhref;
	private SkinButton addbmk;
	private SkinButton addshr;
	private SkinButton share;
	private SkinButton copyeltxt;
	private SkinButton shareeltxt;
	private LinearLayout linear12;
	private SkinButton toelem;
	private SkinAutoCompleteTextView autocomplete1;
	private SkinButton wayback;
	private LinearLayout linear13;
	private SkinButton button3;
	private SkinTextView a4;
	private SkinEditText txtpage;
	private SkinTextView hint;
	private HorizontalScrollView hscroll1;
	private LinearLayout linear7;
	private LinearLayout linear4;
	private SkinTextView a6;
	private SkinEditText pagecode;
	private SkinButton copyurl;
	private SkinButton button4;
	private SkinButton addshortcut;
	private SkinButton sharesite;
	private RadioGroup radiogroup1;
	private SkinRadioButton chatgpt;
	private SkinRadioButton gemini;
	private SkinRadioButton deepseek;
	private SkinRadioButton deepseek2;
	private SkinButton w30;
	private SkinButton w120;
	private SkinButton w240;
	private SkinButton ab1;
	private SkinButton ab3;
	private SkinAutoCompleteTextView fn;
	private SkinButton savehtml;
	private SkinButton mht;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		webview1.setWebViewClient(new WebViewClient() {
			...
			public void onPageFinished(WebView _param1, String _param2) {
				final String _url = _param2;
				// Безпечні локальні посилання на UI-елементи/дані (щоб уникнути гонок або випадкових змін)
				final WebView webView = webview1;
				final TextView propView = prop;
				final TextView pageCodeView = pagecode;
				final Map<String, Object> dataMap = Datas;
				final int currentType = (int)type;
				
				try {
					// 0) Перевірка URL (без NPE)
					String url = "";
					if (webView != null && webView.getUrl() != null) url = webView.getUrl();
					
					if (url.contains("https://chat.deepseek.com") ||
					url.contains("https://gemini.google.com") ||
					url.contains("https://chatgpt.com")) {
						// Показ повідомлення користувачу
						try {
							SketchwareUtil.showMessage(getApplicationContext(), "Вставте текст з буфера обміну");
						} catch (Throwable ignore) { /* не даємо цьому завалити метод */ }
					}
					
					// 1) Перший evaluateJavascript — отримуємо деталі елемента (getJavaScript() має повернути JS, що формує JSON)
					String jsForElement = getJavaScript();
					if (webView != null && jsForElement != null) {
						webView.evaluateJavascript("javascript:" + jsForElement, new ValueCallback<String>() {
							@Override
							public void onReceiveValue(String value) {
								// Повний захист колбеку — нічого не повинно падати вгору
								try {
									// Попередня обробка відповіді JS
									if (value == null || value.equals("null") || value.length() < 2) {
										// Нічого не знайдено
										if (propView != null) {
											propView.setText("No element found");
										}
									} else {
										// JS повертає рядок у лапках — видаляємо крайні лапки і розпаковуємо екрановані лапки
										String json;
										try {
											json = value.substring(1, value.length() - 1).replace("\\\"", "\"");
										} catch (Exception e) {
											json = value; // на випадок дивного формату
										}
										...
										try {
											...
											if (propView != null) propView.setText("Error parsing JSON");
											result = null;
										}
										
										StringBuilder sb = new StringBuilder();
										if (result != null) {
											String t = result.optString("type", "unknown");
											switch (t) {
												case "image":
												sb.append("Format: ").append(result.optString("format", "N/A")).append(" • ");
												sb.append("Resolution: ")
												.append(result.optInt("width", 0)).append("x").append(result.optInt("height", 0))
												.append(" • ");
												break;
												case "video":
												double dur = result.optDouble("duration", -1);
												sb.append("Duration: ").append(dur >= 0 ? String.valueOf(dur) : "N/A").append(" seconds • ");
												break;
												case "container":
												int elems = result.optInt("elements", -1);
												sb.append("Number of elements: ").append(elems >= 0 ? String.valueOf(elems) : "N/A").append(" • ");
												break;
												case "text":
												int len = result.optInt("length", -1);
												sb.append("Length: ").append(len >= 0 ? String.valueOf(len) : "N/A").append(" characters • ");
												sb.append("Font: ").append(result.optString("font", "N/A")).append(" • ");
												break;
												default:
											...
											sb.append("Code size: element = ").append(result.optString("size", "N/A"));
										} else {
											sb.append("No element metadata");
										}
										
										// Безпечне оновлення propView
										if (propView != null) {
											try {
												propView.setText(sb.toString());
											} catch (Throwable ignored) { }
										}
									}
									
									// --- Обчислення розміру сторінки (після оновлення prop) ---
									try {
										String pageText = "";
										
										if (pageCodeView != null) {
											CharSequence seq = pageCodeView.getText();
											if (seq != null && seq.length() > 0) {
												pageText = seq.toString();
											}
										}
										
										if (pageText.isEmpty() && dataMap != null) {
											Object wcodeObj = dataMap.get("wcode");
											if (wcodeObj != null) {
												// безпечне toString
												pageText = String.valueOf(wcodeObj);
											}
										}
										
										double kb = 0;
										if (pageText != null && !pageText.isEmpty()) {
											kb = Math.round(pageText.length() / 10.24d) / 100d;
										}
										
										if (propView != null) {
											String existing = "";
											CharSequence cur = propView.getText();
											if (cur != null) existing = cur.toString();
											propView.setText(existing + ", page = " + kb + " KiB");
										}
									} catch (Throwable t) {
										if (propView != null) {
											CharSequence cur = propView.getText();
											String existing = cur != null ? cur.toString() : "";
											propView.setText(existing + ", page = N/A");
										}
									}
								} catch (Throwable ex) {
									// Логування — дуже корисно при налагодженні
									android.util.Log.e("analyzeWebElement", "Error in element callback", ex);
									if (propView != null) {
										try { propView.setText("Callback error: " + ex.getMessage()); } catch (Throwable ignored) {}
									}
								}
							}
						});
					...
					// 2) Якщо type == 6 — окремо зчитуємо текстове поле сторінки
					if (currentType == 6 && webView != null) {
						final String jsReadTextField =
						"(function() {"
						+ "  var textField = document.querySelector('input[type=\"text\"], txtarea');"
						+ "  if (textField) {"
						...
						webView.evaluateJavascript("javascript:" + jsReadTextField, new ValueCallback<String>() {
							@Override
							public void onReceiveValue(String value) {
								try {
									if (value == null || value.equals("null") || value.length() < 2) {
										// порожнє — нічого не робимо або очищуємо txtarea
										if (txtarea != null) {
											txtarea.setText("");
										}
										return;
									...
									// Розпаковка рядка — безпечна
									String unquoted;
									try {
										unquoted = value.substring(1, value.length() - 1).replace("\\n", "\n").replace("\\\"", "\"");
									} catch (Exception e) {
										unquoted = value;
									}
									
									if (txtarea != null) {
										txtarea.setText(unquoted);
									}
								} catch (Throwable ex) {
									android.util.Log.e("analyzeWebElement", "Error in textfield callback", ex);
									if (txtarea != null) {
										try { txtarea.setText(""); } catch (Throwable ignored) {}
									}
								}
							}
						});
					...
					// Глобальний захист методу — ніяких аварій
					android.util.Log.e("analyzeWebElement", "Unexpected error", outer);
					try {
						if (prop != null) prop.setText("Unexpected error: " + outer.getMessage());
					} catch (Throwable ignored) {}
				}
		...
		addbmk.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				if (href.getText().toString().isEmpty()) {
					buffer = ((!isNotNullOrEmpty(Datas, "href")) ? ("") : (Datas.get("href").toString()));
				} else {
					buffer = href.getText().toString();
				}
				intent.setData(Uri.parse(buffer));
				if (eltext.getText().toString().isEmpty()) {
					buffer = ((!isNotNullOrEmpty(Datas, "text")) ? ("") : (Datas.get("text").toString()));
				} else {
					buffer = eltext.getText().toString();
				}
		...
		button3.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				intent.putExtra("tag", Datas.get("tag").toString());
				startActivity(intent);
			}
		});
		...
		button4.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				intent.setClass(getApplicationContext(), BookmarksActivity.class);
				if (href.getText().toString().isEmpty()) {
					buffer = ((!isNotNullOrEmpty(Datas, "url")) ? ("") : (Datas.get("url").toString()));
				} else {
					buffer = href.getText().toString();
				}
				intent.setData(Uri.parse(buffer));
				if (eltext.getText().toString().isEmpty()) {
					buffer = ((!isNotNullOrEmpty(Datas, "title")) ? ("") : (Datas.get("title").toString()));
				} else {
					buffer = eltext.getText().toString();
				}
				intent.putExtra("n", buffer);
	...
	private void initializeLogic() {
		...
		savehtml.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/icons.ttf"), 0);
		mht.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/icons.ttf"), 0);
		vscroll2.setVisibility(View.GONE);
		a1.setText(getString(R.string.src) + " [▼]");
		a2.setText(getString(R.string.href) + " [▼]");
		a5.setText(getString(R.string.eltext) + " [▼]");
		...
		try{
			Datas = JsonUtils.M(dataHolder.getData("info"));
			...
			try{
				webview1.loadDataWithBaseURL(Datas.get("url").toString(), _correctelHTML(), "text/html", "UTF-8", null);
			}catch(Exception e){
		...
		// Перед finish() у SsuperactivityActivity
		ExtendedDataHolder.getInstance().setData("info", null);
	}
	...
	public String _correctelHTML() {
		if (Datas == null) return "<div>Помилка: немає даних</div>";
		
		String code  = safeGet("code");
		String src   = safeGet("src");
		String href  = safeGet("href");
		
		String html;
		
		// Якщо коду немає — генеруємо базовий шаблон
		if (code.isEmpty()) {
			switch ((int)type) {
				case 5:
				...
				break;
				case 12:
				html = "<img src=\"" + (src.isEmpty() ? href : src) + "\">";
				break;
				default:
				html = "<div>Невідомий тип (" + type + ")</div>";
				break;
			}
		} else {
			html = code;
			...
			// Підміняємо src
			if (!src.isEmpty()) {
				html = html.replaceFirst("src=\"[^\"]*\"", "src=\"" + src + "\"");
			}
			...
			// SVG-обгортка
			if (type == 11) {
				html = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 5000 5000\" " +
...
*/
	...
	public void _getType(final String _c) {
		try {
			...
			if (_c.equals("101") || _c.equals("107") || _c.equals("font") || _c.equals("text") || _c.equals("a") || _c.equals("span") || _c.equals("p") || _c.equals("td") || _c.equals("li")) {
				textview1.setText( getString(R.string.text));
				type = 5;
			}
			if (_c.equals("109") || _c.equals("textarea") || _c.equals("input") || _c.equals("password")) {
			...
			if (_c.equals("105") || _c.equals("106") || _c.equals("108") || _c.equals("img") || _c.equals("image")) {
			...
			if (_c.equals("102")) {
				textview1.setText( getString(R.string.sm_phonenum));
				type = 5;
			}
			if (_c.equals("103")) {
				textview1.setText( getString(R.string.sm_geo));
				type = 5;
			}
			if (_c.equals("104")) {
				textview1.setText( getString(R.string.sm_email));
				type = 5;
			}
	...
	public void _checkblock() {
		// Отримуємо екземпляр нашого блокувальника
		final AdBlocker adBlocker = AdBlocker.getInstance(this);
		
		// Перевіряємо наявність src в мапі даних
		if (Datas != null && Datas.containsKey("src") && Datas.get("src") != null) {
			final String targetUrl = Datas.get("src").toString();
			
			// 1) Оновлюємо стан нашої кнопки "ad" автоматично за допомогою нового методу
			adBlocker.checkAdButton(ad, targetUrl);
			
			// Вішаємо на кнопку слухач подій
			ad.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					// 2) Викликаємо універсальний покращений діалог блокування/розблокування
					// Передаємо Runnable для оновлення тексту кнопки відразу після закриття діалогу
					adBlocker.showBlockDialog(SsuperactivityActivity.this, targetUrl, new Runnable() {
						@Override
						public void run() {
							// Цей код виконається автоматично після успішної дії в діалозі
							adBlocker.checkAdButton(ad, targetUrl);
							
							// Тут також можна викликати ваш метод перезавантаження сторінки, якщо треба:
							// _checkblock();
						}
					});
				}
			...
			// Якщо посилання немає — просто ховаємо кнопку
