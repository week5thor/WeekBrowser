			...
			public void onClick(View _view) {
				if (img.getText().toString().isEmpty()) {
					...
					if (buffer != null && !buffer.isEmpty()) {
						FileDownloader fileDownloader = new FileDownloader(SsuperactivityActivity.this, webview1); fileDownloader.downloadFromUrl(buffer);
					} else {
...
*/
	...
	public void sendChatRequest(int service, String prompt) {
			new Thread(() -> {
					...
					try {
							...
							// Тіло запиту (JSON)
							String jsonInputString = "{\"question\": \"" + prompt + "\"}";
							...
							runOnUiThread(() -> {
									webview1.loadDataWithBaseURL(getIntent().getStringExtra("url"), "Саме тут скоро у Вас чарівним чином з\'явиться текст, якого Ви так бажаєте...", "text/plain", "UTF-8", null);
							});
	...
	public void _summarize(final double _w) {
		...
		if (sf.getString("llm", "").equals("0")) {
			...
			((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", "Please, summarize text to ".concat(((_w == 0) ? ("30 words") : (((_w == 1) ? ("120 words") : (((_w == 2) ? ("240 words") : (((_w == 3) ? ("1 paragraph") : ("3 paragraphs")))))))).concat(" on same language as text. Text:  ")).concat(buffer)));
