	...
	private void initialize(Bundle _savedInstanceState) {
		...
		srhhis.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> _param1, View _param2, int _param3, long _param4) {
				...
				FileUtil.writeFile(MainActivity.this.getFilesDir().getPath() + "/srhhis.json", new Gson().toJson(srcsug));
				_srhupd();
				return true;
			}
		});
...
*/
		...
		dual.setOnLongClickListener(new View.OnLongClickListener() {
			@Override
			public boolean onLongClick(View _view) {
				SketchwareUtil.showMessage(getApplicationContext(), getString(R.string.multiwindow));
				return true;
			}
		});
...
*/
		...
		autocomplete1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				for (String local : srcsug) {
						if (local.toLowerCase().startsWith(_charSeq.toLowerCase())) {
								suggestions.add(local);
						}
				}
				
				// Фоновий потік для онлайн-пропозицій
				new Thread(() -> {
						try {
								URL url = new URL("https://suggestqueries.google.com/complete/search?client=firefox&q=" + URLEncoder.encode(_charSeq, "UTF-8"));
		...
		button1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				final SharedPreferences sf = getSharedPreferences("sf", Context.MODE_PRIVATE);
				if (autocomplete1.getText().toString().contains(".")) {
					...
					if (sf.getString("incognito", "").equals("0")) {
						...
						FileUtil.writeFile(getFilesDir().getPath()+"/srhhis.json", new Gson().toJson(srcsug));
						_srhupd();
					}
					_webViewActions(10, URL);
					closeeditor.performClick();
	...
	private void initializeLogic() {
		...
		try{
			srcsug = new Gson().fromJson(FileUtil.readFile(getFilesDir().getPath()+"/srhhis.json"), new TypeToken<ArrayList<String>>(){}.getType());
		}catch(Exception e){
...
skin.setBG(bml5, 0, false);*/
		...
		try{
			rpl = new Gson().fromJson(FileUtil.readFile(getFilesDir().getPath()+"/replace.json"), new TypeToken<ArrayList<HashMap<String, Object>>>(){}.getType());
		}catch(Exception e){
	...
	public void _ifex() {
		...
		if (sf == null) {
			    ...
			    if (!sf.contains("dvalp")) {
						sf.edit().putString("dvalp", "1").commit();
				}
			    if (!sf.contains("don")) {
						sf.edit().putString("don", Build.VERSION.SDK_INT >= Build.VERSION_CODES.S ? "1" : "0").commit();
				}
		}
...
addshortcut.requestLayout();*/
		...
		forcesearch.getLayoutParams().width = (int) ((Integer.parseInt(sf.getString("button", "")) * getResources().getDisplayMetrics().density * 1.27));
		forcesearch.getLayoutParams().height = (int) (Integer.parseInt(sf.getString("button", "")) * getResources().getDisplayMetrics().density);
		forcesearch.setTextSize( Float.parseFloat(sf.getString("button", "")) / 2.5f);
		forcesearch.requestLayout();
		
		
		
		
		forceurl.getLayoutParams().width = (int) ((Integer.parseInt(sf.getString("button", "")) * getResources().getDisplayMetrics().density * 1.27));
		forceurl.getLayoutParams().height = (int) (Integer.parseInt(sf.getString("button", "")) * getResources().getDisplayMetrics().density);
		forceurl.setTextSize( Float.parseFloat(sf.getString("button", "")) / 2.5f);
		forceurl.requestLayout();
		
		
		
		
		share.getLayoutParams().width = (int) ((Integer.parseInt(sf.getString("button", "")) * getResources().getDisplayMetrics().density * 1.27));
		share.getLayoutParams().height = (int) (Integer.parseInt(sf.getString("button", "")) * getResources().getDisplayMetrics().density);
		share.setTextSize( Float.parseFloat(sf.getString("button", "")) / 2.5f);
		share.requestLayout();
...
*/
			...
			new Thread(new Runnable() {
					...
					public void run() {
							try{
									...
									//SketchwareUtil.showMessage(getApplicationContext(), "шо то взагалі сі творе? Жестб мене хапануло");
							}
					}
			}).start();
		 ...
		 View.LAYER_TYPE_HARDWARE:View.LAYER_TYPE_SOFTWARE, null);
		    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			    try {
				        ...
				        //e.printStackTrace(); // лог, але не краш
				    }
			    }
		    ...
		    if (sf.getString("incognito", "").equals("1")) {
			    ...
			    if (Build.VERSION.SDK_INT >= 21) {
				        try {
					            ...
					            method.invoke(CookieManager.getInstance(), _w, false);
					        } catch (Exception e) {
					            e.printStackTrace(); // Безпечне ігнорування помилки
					        }
				    }
		} else {
			    ...
			    if (Build.VERSION.SDK_INT >= 21) {
				        try {
					            Method method = CookieManager.class.getMethod("setAcceptThirdPartyCookies", WebView.class, boolean.class);
					            method.invoke(CookieManager.getInstance(), _w, true);
					        } catch (Exception e) {
					            e.printStackTrace(); // Безпечне ігнорування помилки
					        }
				    }
	...
	public String _getMultilink() {
		...
		if (tab.size() <= 100) {
			    try {
				        String json = new Gson().toJson(tab2);
				        return "https://multilink2.weekbrowser.com/" + (is1window?"w":"") + encodeUrl(json);
				    } catch (Exception e) {
	...
	public void _onpagefinish(final WebView _w, final boolean _t) {
		...
		//TimerTask tim;
		_w.loadUrl("javascript:(function(){document.addEventListener('contextmenu',function(e){try{var a=e.target,b=a.outerHTML,c=a.innerText||'',d=a.id||'',f=a.src||'',g='',h=a.tagName.toLowerCase(),i=document.body.innerText||'',j=document.documentElement.outerHTML,k=location.href,l=document.title;if(a.tagName==='A')g=a.href;else{var m=a.closest('a');if(m)g=m.href;}var n={code:b,text:c,src:f,href:g,wtext:i,wcode:j,taid:d,url:k,type:h,title:l};window.Vidro.processElementInfo(JSON.stringify(n));}catch(o){console.error('WeekBrowser Supermenu: Error processing element: ',o);}},true);})();");
		if (sf.getString("skipvideoad", "").equals("1")) {
			_w.loadUrl("javascript:(function(){var a=false;function b(){var c=document.querySelectorAll('video,[src*=\\.mp4],[src*=\\.webm],[src*=\\.ogv],[type*=video]');for(var d=0;d<c.length;d++){var e=c[d];if(typeof e.currentTime!=='undefined')e.currentTime=65535;}}function f(){var g=new RegExp(\""+sf.getString("videopatterns","")+"\");if(g.test(document.body.innerText)&&!a){a=true;setTimeout(function(){a=false;},2000);b();}}function h(){return document.querySelector('video')||document.querySelector('[src*=\\.mp4]')||document.querySelector('[src*=\\.webm]')||document.querySelector('[src*=\\.ogv]')||document.querySelector('[type*=video]');}if(h()){var i=new MutationObserver(function(j){j.forEach(function(k){if(k.type==='childList'||k.type==='characterData')f();});});i.observe(document.body,{childList:true,subtree:true,characterData:true,characterDataOldValue:true});f();}})();");
		}
		try{
			if (sf.getString("iframer", "").equals("1") && !_w.getUrl().contains("accounts.google.com")) {
				_w.loadUrl("javascript:(function(){var a=document.querySelectorAll('iframe');if(!a.length)return;a.forEach(function(b){var c=b.src||b.getAttribute('data-src');if(!c)return;var d=document.createElement('a');d.href=c;var e=d.href,f=document.createElement('div');f.style.position='relative';b.parentNode.insertBefore(f,b);f.appendChild(b);var g=document.createElement('a');g.href=e;g.target='_blank';g.textContent='[↗]';g.style.position='absolute';g.style.bottom='-25px';g.style.right='10px';g.style.backgroundColor='#0ff';g.style.color='#000';g.style.padding='12px';g.style.fontSize='24px';g.style.cursor='pointer';g.style.textDecoration='none';g.style.borderRadius='4px';g.style.zIndex='9999999';f.appendChild(g);});})();");
			}
		}catch(Exception e){
...
*/
	...
	public void _tabupd() {
		tabs.setText("[" + String.valueOf(tab.size()) + "]•");
	}
				...
				public boolean onLongClick(View _view) {
					AlertDialog.Builder builder = new AlertDialog.Builder(MainActivity.this);
					        builder.setTitle(_data.get((int)_position).get("name").toString())
					               .setItems(new CharSequence[]{getString(R.string.add_tab), getString(R.string.add_bg_tab), getString(R.string.add_tab_incognito), getString(R.string.add_tab_bg_incognito)
						,getString(R.string.close_tab), getString(R.string.close_other), getString(R.string.dup_tab), getString(R.string.unload_tab), getString(R.string.unload_other), getString(R.string.add_to_bookmark), getString(R.string.create_shortcut), getString(R.string.close_block)},
