...
public class JSutil {
	...
	private static String generateGradient(String colors, int gradientType, int segments, int sharpness) {
		...
		for (int i = 0; i < segments; i++) {
			...
			if(sharpness==0){
            	gradient.append(convertColorFormat(colorArray[rnd.nextInt(colorArray.length)])).append(", ");
			}else{
		...
		switch (gradientType) {
			...
			default: return "linear-gradient(to bottom, " + gradient.toString() + ")";
		}
	}
	...
	public static String replaceInstruction(String jsCode) {
		if(!jsCode.startsWith("javascript:"))jsCode="javascript:"+jsCode;
